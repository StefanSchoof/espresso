...
describe('main', () => {
  beforeAll(() => {
    window.fetch = jest.fn(async () => await Promise.resolve({ ok: true })) as any
  })
  ...
  describe('click', () => {
    ...
    test('show error if request after on click is not ok', async () => {
      window.fetch = jest.fn(async () => await Promise.resolve({ ok: false, text: async () => await Promise.resolve('Not working') })) as any
