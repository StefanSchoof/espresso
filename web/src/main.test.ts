...
describe("main", () => {
  ...
  describe("click", () => {
    ...
    test("send request after on click", async () => {
      ...
      ).toBe("Schalte Maschine an");
      await immediate();

      expect(window.fetch).toHaveBeenCalledWith(
        expect.stringContaining("api/on"),
        { method: "POST" },
      );
      expect(
        (document.getElementById("status") as HTMLSpanElement).textContent,
    ...
    test("send request after off click", async () => {
      ...
      expect(
        (document.getElementById("status") as HTMLSpanElement).textContent,
      ).toBe("Schalte Maschine aus");
      await immediate();

      expect(window.fetch).toHaveBeenCalledWith(
        expect.stringContaining("api/off"),
        { method: "POST" },
      );
      expect(
        (document.getElementById("status") as HTMLSpanElement).textContent,
      ...
      window.fetch = jest.fn(
        async () =>
          await Promise.resolve({
            ok: false,
            text: async () => await Promise.resolve("Not working"),
          }),
      ) as any;
